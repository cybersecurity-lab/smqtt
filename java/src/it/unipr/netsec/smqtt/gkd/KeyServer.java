...
import it.unipr.netsec.smqtt.gkd.method.StaticGKDService;
import it.unipr.netsec.smqtt.gkd.method.UpdateGKDService;
 ...
 * <ul>
 * <li><i>STATIC</i> - For each group a static group key is generated by the KS and distributed to each authorized client
 * that requests to join the group. </li>
 * <li><i>UPDATE</i> - for each group a group key is distributed by the KS to authorized client. Each time a new client
 ...
 * </ul>
 * <p>
 * Regardless the selected group key distribution method, all key material exchanged between the KS and the MQTT clients
...
public class KeyServer {
	 ...
	 * @throws IOException */
	public KeyServer(String serverId, String broker) throws IOException {
	 ...
	 * @throws IOException */
	public KeyServer(String serverId, String broker, String username, String passwd) throws IOException {
		/*switch (GKD_METHOD) {
	...
	private void processReceivedMessage(String topic, byte[] payload) {
		try {
			...
			if (!topicFields[0].equals(TOPIC_GKD)) {
				if (KeyServer.DEBUG||KeyServer.VERBOSE) log("processReceivedMessage(): not for key distribution: discarded");
				return;
			}
			...
			try {
				...
				if (KeyServer.DEBUG||KeyServer.VERBOSE) log("processReceivedMessage(): wrong GKD type: "+topicFields[1]);
				return;				
			}
