...
In particular, the Secure MQTT protocol here implemented performs group key management based on the presence of a Key Server (KS).
...
This implementation can be easily tested also by launching the test.SMqttTest program that runs a KS and n SMQTT clients. One of the three GKD methods can be selected (default is 2=update).
...
The implemented KS actually support three different group key management mechanisms:
...
The KS can be run on a Java 10+ JVM. It just requires a standard MQTT (e.g. Eclipse mosquitto) running and reachable at a given IP address:
``` bash
...
When the MQTT broker and KS are up, you can run a SMQTT client as follows:
``` bash
